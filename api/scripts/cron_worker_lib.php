...
if (!function_exists('cw_log')) {
    function cw_log(string $message): void
    {
        $ts = date('Y-m-d H:i:s');
        $line = "[{$ts}] {$message}\n";
        // HTTP triggers must return clean JSON, so progress lines are echoed
        // only for CLI runs (cPanel cron output and manual diagnostics).
        if (PHP_SAPI === 'cli') {
            echo $line;
        }

        @file_put_contents(cw_log_file(), $line, FILE_APPEND);
    }
}

if (!function_exists('cw_log_file')) {
    function cw_log_file(): string
    {
        $configured = getenv('CW_LOG_FILE');
        if (is_string($configured) && trim($configured) !== '') return $configured;
        return __DIR__ . '/../logs/academic-attendance-worker.log';
    }
}
...
if (!function_exists('cw_cron_state_path')) {
    function cw_cron_state_path(): string
    {
        return __DIR__ . '/../logs/cpanel-attendance-cron-state.json';
    }
}

if (!function_exists('cw_cron_state_read')) {
    function cw_cron_state_read(string $statePath = ''): array
    {
        $path = $statePath !== '' ? $statePath : cw_cron_state_path();
...
if (!function_exists('cw_cron_state_write')) {
    function cw_cron_state_write(array $state, string $statePath = ''): bool
    {
        $path = $statePath !== '' ? $statePath : cw_cron_state_path();
...
if (!function_exists('cw_cron_due_flags')) {
    /**
     * Decide which gated work is due for one invocation.
     *
     * The status pass always runs; the five-minute and daily passes are
     * remembered in the state file so the scheduler only needs to be registered
     * once per minute.
     */
    function cw_cron_due_flags(array $state, bool $forceFiveMinute = false, int $now = 0): array
    {
        $now = $now > 0 ? $now : time();
        ...
        $dailyDue = $lastDailyDate !== date('Y-m-d', $now) && date('H:i', $now) >= '00:05';

        return [
            'academic_sync_due' => $forceFiveMinute || $lastAcademicAt <= 0 || ($now - $lastAcademicAt) >= $fiveMinuteSeconds,
            'attendance_generation_due' => $forceFiveMinute || $lastGenerationAt <= 0 || ($now - $lastGenerationAt) >= $fiveMinuteSeconds,
...
if (!function_exists('cw_run_scheduled_tasks')) {
    /**
     * Run the scheduled attendance work once.
     *
     * Shared by the cPanel CLI entry point (scripts/cpanel-attendance-cron.php)
     * and the token-protected HTTP trigger (api/cron), so hosting without CLI
     * cron can still keep attendance, absence flags and Web Push alerts live.
     *
     * @return array{ok:bool,exit_code:int,due:array<string,bool>,academic:?array,generation:?array,statuses:?array,state_saved:bool,errors:array<int,string>}
     */
    function cw_run_scheduled_tasks(mysqli $mysqli, bool $forceFiveMinute = false): array
    {
        $statePath = cw_cron_state_path();
        $state = cw_cron_state_read($statePath);
        $now = time();
        $today = date('Y-m-d', $now);
        $due = cw_cron_due_flags($state, $forceFiveMinute, $now);
        ...
        try {
            // Run this before attendance generation so the correct semester and
            // school year are active. The daily safety pass shares the call.
            if ($due['academic_sync_due'] || $due['daily_safety_due']) {
                $academic = cw_daily_academic_update($mysqli);
                $result['academic'] = $academic;
                if (!$academic['ok']) {
                    $exitCode = 1;
                    $result['errors'][] = implode(' | ', $academic['errors']);
                    cw_log('[cPanelCron][AcademicStatusSync] failed: ' . implode(' | ', $academic['errors']));
                } else {
                    $state['last_academic_sync_at'] = $now;
                    if ($due['daily_safety_due']) {
                    ...
                    cw_log(
                        ...
                        . ' daily_safety=' . ($due['daily_safety_due'] ? 'yes' : 'no')
                    );
                }
            }

            if ($due['attendance_generation_due']) {
                $generation = cw_generate_attendance_records($mysqli);
                $result['generation'] = $generation;
                if (!$generation['ok']) {
                    $exitCode = 1;
                    $result['errors'][] = implode(' | ', $generation['errors']);
            ...
            // The status pass runs on every invocation so a one-minute
            // scheduler keeps absence flags and Web Push alerts close to real
            // time. Standard shared-hosting cron cannot guarantee the former
            // Windows worker's ten-second interval.
            $statuses = cw_process_attendance_statuses($mysqli);
            $result['statuses'] = $statuses;
            if (!$statuses['ok']) {
                $exitCode = 1;
                $result['errors'][] = implode(' | ', $statuses['errors']);
        ...
        if ($stateChanged) {
            $result['state_saved'] = cw_cron_state_write($state, $statePath);
